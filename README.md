...
使用 `-s` 或 `--silent` 参数启用静默模式：
- ✅ 不输出任何信息（包括成功和错误信息）
- ✅ 复制成功时退出码为 0
- ✅ 复制失败时退出码为 1
- ✅ 适合在脚本中使用，通过检查剪贴板内容或退出码判断结果
...
```bash
# Batch
clip.exe -s data.txt && echo 成功 || echo 失败

# PowerShell
if (.\clip.exe -s data.txt) { Write-Host "已复制" }
